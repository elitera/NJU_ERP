<template>
  <el-container style="height: 100%; border: 1px solid #eee">
    <el-aside width="250px" style="background-color: #545c64">
      ...
      <el-menu :default-active="getActivePath" class="el-menu-vertical-demo" background-color="#fff" text-color="#000000DE" @open="handleOpen" @close="handleClose" active-text-color="#7e57c2" router>
        ...
        <el-submenu index="2" v-if="permit(PATH.INVENTORY_OPERATION.requiresAuth)">
          ...
          <el-menu-item :index="PATH.INVENTORY_OPERATION.path" v-if="permit(PATH.INVENTORY_OPERATION.requiresAuth)">
            <i class="el-icon-receiving"></i>
        ...
        <el-submenu index="3" v-if="permit(PATH.CUSTOMER_VIEW.requiresAuth)">
          ...
          <el-menu-item :index="PATH.PROMOTION.path" v-if="permit(PATH.PROMOTION.requiresAuth)">
            <i class="el-icon-user"></i>
        ...
        <el-submenu index="5" v-if="permit(PATH.EMPLOYEE_VIEW.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">员工管理</span>
          </template>
          <el-menu-item :index="PATH.EMPLOYEE_VIEW.path" v-if="permit(PATH.EMPLOYEE_VIEW.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">员工信息</span>
          </el-menu-item>
          <!--          <el-menu-item-->
          <!--              :index="PATH.EMPLOYEE_SALARY.path"-->
          <!--              v-if="permit(PATH.EMPLOYEE_SALARY.requiresAuth)"-->
          <!--          >-->
          <!--            <i class="el-icon-receiving"></i>-->
          <!--            <span slot="title">薪资制定</span>-->
          <!--          </el-menu-item>-->
          <el-menu-item :index="PATH.EMPLOYEE_CLOCK_IN.path" v-if="permit(PATH.EMPLOYEE_CLOCK_IN.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">员工打卡查询</span>
          </el-menu-item>
        </el-submenu>

        <el-submenu index="6" v-if="permit(PATH.ACCOUNT_MANAGEMENT.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">账户管理</span>
          </template>
          <el-menu-item :index="PATH.ACCOUNT_MANAGEMENT.path" v-if="permit(PATH.ACCOUNT_MANAGEMENT.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">账户管理</span>
          </el-menu-item>
        </el-submenu>

        <el-submenu index="7" v-if="permit(PATH.CHARGE_BILL_MAKE.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">表单制定</span>
          </template>
          <el-menu-item :index="PATH.CHARGE_BILL_MAKE.path" v-if="permit(PATH.CHARGE_BILL_MAKE.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">收款单</span>
          </el-menu-item>
          <el-menu-item :index="PATH.PAYMENT_BILL_MAKE.path" v-if="permit(PATH.PAYMENT_BILL_MAKE.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">付款单</span>
          </el-menu-item>
          <el-menu-item :index="PATH.CASH_BILL_MAKE.path" v-if="permit(PATH.CASH_BILL_MAKE.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">现金费用单</span>
          </el-menu-item>
          <el-menu-item :index="PATH.SALARY_SHEET_MAKE.path" v-if="permit(PATH.SALARY_SHEET_MAKE.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">工资单</span>
          </el-menu-item>
        </el-submenu>

        <el-submenu index="8" v-if="permit(PATH.CONDITION_SHEET.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">表单查询</span>
          </template>
          <el-menu-item :index="PATH.CONDITION_SHEET.path" v-if="permit(PATH.CONDITION_SHEET.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">经营状况表</span>
          </el-menu-item>
          <el-menu-item :index="PATH.PROGRESS_SHEET.path" v-if="permit(PATH.PROGRESS_SHEET.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">经营历程表</span>
          </el-menu-item>
          <el-menu-item :index="PATH.SALE_SHEET.path" v-if="permit(PATH.SALE_SHEET.requiresAuth)">
            <i class="el-icon-receiving"></i>
            <span slot="title">销售明细表</span>
          </el-menu-item>
        </el-submenu>
        <el-submenu index="9" v-if="permit(PATH.PUNCH.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">每日打卡</span>
          </template>
          <el-menu-item :index="PATH.PUNCH.path" v-if="permit(PATH.PUNCH.requiresAuth)">
            <i class="el-icon-receiving"></i>
