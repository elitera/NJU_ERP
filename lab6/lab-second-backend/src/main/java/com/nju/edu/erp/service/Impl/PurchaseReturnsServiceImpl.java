...
public class PurchaseReturnsServiceImpl implements PurchaseReturnsService {
    ...
    public List<String> findSheetIdByOperator(String operator) {
        return purchaseReturnsSheetDao.findSheetIdByOperator(operator);
    }

    @Override
    public List<String> findSheetIdByTime(Date beginDate, Date endDate) {
        return purchaseReturnsSheetDao.findSheetIdByTime(beginDate, endDate);
    }

    @Override
    public List<String> findSheetIdBySomeConditions(BusinessHistorySheetSearchCriteriaPO searchCriteriaPO) {
        return purchaseReturnsSheetDao.findSheetIdBySomeConditions(searchCriteriaPO);
    }

    @Override
    public PurchaseReturnsSheetVO getPurchaseRetuenSheetById(String id) {
        ...
        List<PurchaseReturnsSheetContentPO> contentPO = purchaseReturnsSheetDao.findContentByPurchaseReturnSheetId(id);
        PurchaseReturnsSheetVO pVO = new PurchaseReturnsSheetVO();
        BeanUtils.copyProperties(purchaseReturnsSheetPO, pVO);
        List<PurchaseReturnsSheetContentVO> purchaseReturnSheetContentVOList = new ArrayList<>();
        for (PurchaseReturnsSheetContentPO content : contentPO) {
            PurchaseReturnsSheetContentVO pContentVO = new PurchaseReturnsSheetContentVO();
            BeanUtils.copyProperties(content, pContentVO);
            purchaseReturnSheetContentVOList.add(pContentVO);
        }
        pVO.setPurchaseReturnsSheetContent(purchaseReturnSheetContentVOList);
        return pVO;
    }
}